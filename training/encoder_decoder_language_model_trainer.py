...
class EncoderDecoderLanguageModelTrainer(BaseTrainer):
    ...
    def create_functions(self):
        ...
        def cross_entropy_batch_loss(params, step_rng, batch, training: bool):
            targets, decoder_tokens, encoder_tokens = batch
            _, decoder_hidden_states, logits = self.model.apply(
                {'params': params}, decoder_tokens, training, encoder_input=encoder_tokens, rngs={'dropout': step_rng},
            )
        ...
        def cross_entropy_batch_loss_and_acc(params, batch):
            targets, decoder_tokens, encoder_tokens = batch
            _, decoder_hidden_states, logits = self.model.apply(
                {'params': params}, decoder_tokens, False, encoder_input=encoder_tokens,
            )
            logits = jnp.reshape(logits, (-1, logits.shape[-1]))    # (batch_size * seq_length, vocab_size)
            targets = jnp.reshape(targets, (-1))                    # (batch_size * sequence_length)
            loss = cross_entropy_loss(logits, targets)
